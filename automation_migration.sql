...
declare v_phone text; v_digits text; v_cust public.customers; v_code text;
begin
  if p_phone is null or btrim(p_phone) = '' then raise exception 'Phone number is required'; end if;
  v_digits := regexp_replace(p_phone, '[^0-9]', '', 'g');
  if length(v_digits) = 13 and left(v_digits,3) = '880' then v_phone := '0' || substring(v_digits from 4);
  elsif length(v_digits) = 10 and left(v_digits,1) = '1' then v_phone := '0' || v_digits;
  else v_phone := v_digits; end if;
  if length(v_phone) < 10 then raise exception 'Phone number is not valid'; end if;
    ...
    update public.customers
       set is_member = true,
           ...
           name = case when (name is null or btrim(name) = '' or name = phone)
                        and p_name is not null and btrim(p_name) <> '' then p_name else name end
...
declare v_set public.shop_settings; v_points numeric := 0; v_total numeric := 0; v_tier text := 'silver';
begin
  ...
  update public.customers
     set loyalty_points = coalesce(loyalty_points,0) + v_points,
         ...
         last_visit_at = now()
   where id = p_customer_id returning total_spent into v_total;

  if v_total >= coalesce(v_set.tier_platinum_at, 50000) then v_tier := 'platinum';
  elsif v_total >= coalesce(v_set.tier_gold_at, 20000) then v_tier := 'gold'; end if;
  ...
  if v_points > 0 then
    ...
    values (p_customer_id, p_sale_id, v_points, 'earn', 'Earned from sale', auth.uid());
  end if;
  return v_points;
end; $$;
...
returns numeric language plpgsql security definer set search_path = public as $$
declare v_set public.shop_settings; v_have numeric := 0; v_taka numeric := 0;
begin
  ...
  if v_have < p_points then raise exception 'Not enough points (has %, requested %)', v_have, p_points; end if;
  if p_points < coalesce(v_set.min_points_to_redeem, 50) then
    raise exception 'At least % points are required', v_set.min_points_to_redeem;
  end if;

  v_taka := p_points * coalesce(v_set.point_value_taka, 1);
  update public.customers set loyalty_points = coalesce(loyalty_points,0) - p_points where id = p_customer_id;
  insert into public.loyalty_tx (customer_id, points, type, note, created_by)
  values (p_customer_id, -p_points, 'redeem', 'Used as discount', auth.uid());
  return v_taka;
end; $$;
grant execute on function public.redeem_points(uuid, numeric) to authenticated;

-- 8) Backfill: everyone with a phone becomes a member
update public.customers
   set is_member = true,
